...
public class Servicio : System.Web.Services.WebService
{
    ...
    public string guardar(int dpi, string nombre,string apellido,string nit,int tel,string dir,int numT,string usu,string contras,string Sul)
    {
        SqlCommand comando = new SqlCommand("Insert into Cliente(DPI,Nombre,Apellidos,Nit,Telefono,Direccion,NumTarjeta,Usuario,Contra,Suc) values(@DPI, @Nombre,@Apellido,@Nit,@Tel,@dir,@numt,@usu,@contras,@Sucursal)", con);
        ...
        comando.Parameters.AddWithValue("Sucursal", Sul);
        try
        {
            con.Open();
    ...
    [WebMethod]
    public string modificarCl(int DPI, string nombre, string apellido, string nit, int telefono, string direccion, int tarjeta, string usuario,string contraseña,string suc)
    {
        SqlCommand comando = new SqlCommand("Update Cliente set DPI = " + DPI + ", Nombre = '" + nombre + "', Apellidos = '" + apellido + "', Nit = '" + nit + "',Telefono = " + telefono + ", Direccion = '" + direccion + "', NumTarjeta = " + tarjeta + ", Usuario = '" + usuario + "',Contra = '" + contraseña + "',Suc = '" + suc + "' where DPI = " + DPI + "", con);
        //comando.Parameters.AddWithValue("DPI", DPI);
        //comando.Parameters.AddWithValue("nombre", nombre);
        //comando.Parameters.AddWithValue("apellido", apellido);
        //comando.Parameters.AddWithValue("nit",nit);
    ...
    public List<string> ModificarCliente(string usuario, string contraseña)
    {
        ...
        while (reader.Read())
        {
            ...
            a.Add(Convert.ToString(reader["Suc"]));
                   }
        codigos = cod;
        conectar.Close();
    ...
    public void AgregarFoto(string foto, int paquete)
    {
        ...
        SqlConnection conectar = new SqlConnection(enlace);
        conectar.Open();
        SqlCommand cmd = new SqlCommand(String.Format("update Paquete set estado=1, Foto= '" + foto + "' where codPaquete="+paquete+""), conectar);
        try
        {
            cmd.ExecuteNonQuery();
        }
    ...
    [WebMethod]
    public void AgregarPrecio(decimal precio, int paquete)
    {
        string enlace = @"Data source = JORGE8A\SQLExpress; Initial Catalog = QuetzalExpre ; Integrated Security= SSPI; Trusted_Connection = Yes";

        SqlConnection conectar = new SqlConnection(enlace);
        conectar.Open();
        SqlCommand cmd = new SqlCommand(String.Format("update Paquete set Estado=2, Precio= " + precio + " where codPaquete like '%{0}%'", paquete), conectar);
        try
        {
            cmd.ExecuteNonQuery();
        }
    ...
    public List<string> BuscarEmpleado(int codigorr)
    {
        int cod = 0;
        List<string> a = new List<string>();
        string enlace = @"Data source = JORGE8A\SQLExpress; Initial Catalog = QuetzalExpre ; Integrated Security= SSPI; Trusted_Connection = Yes";
        SqlConnection conectar = new SqlConnection(enlace);
        conectar.Open();
        SqlCommand cmd = new SqlCommand(String.Format("Select * from Empleado where codEmpleado =" + codigorr + ""), conectar);
        SqlDataReader reader = cmd.ExecuteReader();
        while (reader.Read())
        {
            a.Add(Convert.ToString(reader["Apellido"]));
            a.Add(Convert.ToString(reader["Nombre"]));
            a.Add(Convert.ToString(reader["Sueldo"]));
            a.Add(Convert.ToString(reader["Sucursal"]));
            a.Add(Convert.ToString(reader["Departamento"]));
        }
        codigos = cod;
        conectar.Close();
    ...
    public List<string> Despedir(int codigorr)
    {
        int cod = 0;
        List<string> a = new List<string>();
        string enlace = @"Data source = JORGE8A\SQLExpress; Initial Catalog = QuetzalExpre ; Integrated Security= SSPI; Trusted_Connection = Yes";
        ...
        while (reader.Read())
        {
            ...
            a.Add(Convert.ToString(reader["Departamento"]));
            a.Add(Convert.ToString(reader["Usuario"]));
            a.Add(Convert.ToString(reader["Contraseña"]));
    ...
    //[WebMethod]
    //public string DespedirEmpleado(int cod, string nombre, string apellido, decimal sueldo, string sucursal, string dep, string usuario, string contraseña)
    //{
    //    SqlCommand comando = new SqlCommand("Update Despedido set codEmpleado = " + cod + ", Nombre = '" + nombre + "', Apellido = '" + apellido + "', Sueldo = " + sueldo + ", Sucursal = '" + sucursal + "', Departamento = '" + dep + "', Usuario = '" + usuario + "', Contra = '" + contraseña + "' where codEmpleado = " + cod + "", con);
    ...
    public string DespedirEmpleado(int cod, string nombre, string apellido, double sueldo, string sucursal, string dep, string usuario, string contraseña)
    {
        SqlCommand comando = new SqlCommand("Insert into Despedido(codEmpleado,Nombre,Apellido,Sueldo,Sucursal,Departamento,Usuario,Contra) values(@cod, @nombre,@apellido,@sueldo,@sucursal,@dep,@usuario,@contraseña)", con);
    ...
    [WebMethod]
    public void ConfirmarEstado(int estado)
    {
        SqlCommand comando = new SqlCommand("Update Paquete set estado = 3 where codPaquete ='" + estado + "' ", con);
        try
        {
            con.Open();
            comando.ExecuteNonQuery();
